...
cdef np.ndarray[double, ndim=2, mode='c'] _run(double ND_mean, double ND_sd, int XT, int YT, int XR, int YR, str PM_type, bool default_polarity_markers, 
	...
	# Declare empty list in which to store sequential 's' matrices
	A = []
	...
	while loop_count < num_loops:
		...
		# Save 's' to 'A' if loop_count is divisible by (num_loops/125) (allows 
		# creation of a 5s GIF at 25fps)
		if loop_count%(num_loops/125) == 0:
			A.append(np.copy(s))
	...
	# Create and save record array of 'A'
	r = np.core.records.fromrecords(A)
	np.save('r.npy', r)
